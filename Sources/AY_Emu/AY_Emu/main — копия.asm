...
;	sts		CLKPR,r16		;делитель =1 и вывод clk
	...
	;Ждем после переключения как указано по даташиту
;	ldi     r20, 200
;wait1:  
;	ldi     r21, 200
;wait2:  
...
;	; 2. Unlock protected registers (LGT specific)
	...
	ldi     r16, 0xF0       ; Максимальное значение калибровки
	sts     OSCCAL, r16
...
LOOP2:
	...
	; ИНИЦИАЛИЗАЦИЯ СОСТОЯНИЯ ГЕНЕРАТОРА ОГИБАЮЩЕЙ ДЛЯ AY0
	ldi     ZL, low(EnvState_AY0)
	ldi     ZH, high(EnvState_AY0)
	ldi     r16, 0x80        ; TabE = 0x80 (генератор огибающей выключен)
	clr     r17              ; TabP = 0
	clr     r18              ; EVal = 0
	st      Z+, r16
	st      Z+, r17
	...
	; ИНИЦИАЛИЗАЦИЯ СОСТОЯНИЯ ГЕНЕРАТОРА ОГИБАЮЩЕЙ ДЛЯ AY1
	ldi     ZL, low(EnvState_AY1)
	ldi     ZH, high(EnvState_AY1)
	st      Z+, r16
	st      Z+, r17
	st      Z+, r18
	...
	mov		NoiseAddon_AY0,ZH	; load default value = 1 to high bit of noise generator
	mov		NoiseAddon_AY1,ZH
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	lds		YL,AY0_REG13	; load envelope shape register value to TabE
	ldd		TabE,Y+0x10		; get envelope code from SRAM
	ldi		TabP,0x1F		; set counter for envelope period
	andi	TNLevel_AY0,0x7F		; clear envelope shape change flag
...
CH_C_NO_CHANGE_AY1:
	...
	; === СОХРАНЕНИЕ СОСТОЯНИЯ AY1 ===
	ldi     YL, low(EnvState_AY1) 
	st      Y, TabE         
	std     Y+1, TabP       
	std     Y+2, EVal 

	; === СМЕШИМАНИЕ ВЫХОДОВ AY0 и AY1 ===
	ldd     TMP, Y+5
	add     OutA, TMP
	ldd     TMP, Y+6
	add     OutC, TMP
...
.org	0x0251
EnvState_AY0:   .byte 3  ; [TabE, TabP, EVal] для AY0
EnvState_AY1:   .byte 3  ; [TabE, TabP, EVal] для AY1
